...
# slow-4G first paint stays under budget. Rewrites app/fonts/*.woff2 in place; safe to run again.
...
for font in "$dir"/*-arabic-arabic-*.woff2; do subset "$font" --unicodes="$arabic"; done
for font in "$dir"/*-latin-*.woff2; do subset "$font" --unicodes="$latin"; done

# Header wordmark only (every page, both locales): three letters instead of the whole Naskh face.
python -m fontTools.subset "$dir/noto-naskh-arabic-arabic-400-normal.woff2" --text="وعد" \
  --flavor=woff2 --no-hinting --output-file="$dir/noto-naskh-arabic-wordmark.woff2"
